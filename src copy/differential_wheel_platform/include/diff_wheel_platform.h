...
#ifndef DIFF_WHEEL_PLATFORM_H_
#define DIFF_WHEEL_PLATFORM_H_
...
public:
	...
	TeleopVesc(int no_of_vesc, int no_of_dof)
	{
		...
		// Publisher
		vesc_cmd_get_speeds = nh_.advertise<std_msgs::Bool>("commands/motor/get_speeds", 10);
		...
		//vesc_sensor_speeds_= nh_.subscribe<vesc_msgs::VescGetSpeeds>("sensors/speeds", 10, &TeleopVesc::speedsCallback, this);
	}

	int joy_cont_mode;

	std_msgs::Bool enable;
	ros::Publisher vesc_cmd_get_speeds, vesc_cmd_alive, vesc_cmd_speed, vesc_cmd_current, vesc_cmd_brake;
...
private:
	...
	//void speedsCallback(const vesc_msgs::VescGetSpeeds::ConstPtr& speeds_msg);

	ros::NodeHandle nh_;
	ros::Subscriber joy_sub_;
	ros::Subscriber vesc_sensor_core_, vesc_sensor_speeds_;
};
