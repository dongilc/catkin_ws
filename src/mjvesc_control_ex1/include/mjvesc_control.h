...
#ifndef MJVESC_CONTROL_H_
#define MJVESC_CONTROL_H_
...
public:
	...
	mjvesc_msgs::MJVescSetCommand cmd_msg;
	mjvesc_msgs::MJVescSetCustomApp custom_tx_msg;
	...
	TeleopVesc(const int no_of_vesc, const std::string port)
	{
		...
		vesc_cmd_set_customs = nh_.advertise<mjvesc_msgs::MJVescSetCustomApp>(topic_name, 10);
		...
		vesc_cmd_speed = nh_.advertise<mjvesc_msgs::MJVescSetCommand>(topic_name, 10);
		topic_name = port + "/commands/motor/duty_cycle";
		vesc_cmd_duty = nh_.advertise<mjvesc_msgs::MJVescSetCommand>(topic_name, 10);
		topic_name = port + "/commands/motor/current";
		vesc_cmd_current = nh_.advertise<mjvesc_msgs::MJVescSetCommand>(topic_name, 10);
		topic_name = port + "/commands/motor/position";
		vesc_cmd_position = nh_.advertise<mjvesc_msgs::MJVescSetCommand>(topic_name, 10);
		topic_name = port + "/commands/motor/brake";
		vesc_cmd_brake = nh_.advertise<mjvesc_msgs::MJVescSetCommand>(topic_name, 10);

		// Subscriber
		topic_name = port + "/sensors/core";
		vesc_sensor_core_ = nh_.subscribe<mjvesc_msgs::MJVescStateStamped>(topic_name, 10, &TeleopVesc::stateCallback, this);
		topic_name = port + "/sensors/customs";
		vesc_sensor_customs_= nh_.subscribe<mjvesc_msgs::MJVescGetCustomApp>(topic_name, 10, &TeleopVesc::customsCallback, this);
	}
...
private:
	void stateCallback(const mjvesc_msgs::MJVescStateStamped::ConstPtr& state_msg);
	void customsCallback(const mjvesc_msgs::MJVescGetCustomApp::ConstPtr& custom_rx_msg);
