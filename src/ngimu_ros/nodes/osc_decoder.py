...
def decode(data):
    messages = []
    #rospy.loginfo("data_length:%d", len(data))
...
def _process_packet(data, timestamp, messages):
    if data[0] == 35:  # if packet is a bundle ("#" = ASCII 35)
        ...
        #rospy.loginfo("pp_contents:%s",len(contents))
        #rospy.loginfo("pp_length_contents:%d",len(contents))
        for content in contents:
            #rospy.loginfo("pp_content:%s", content)
            #rospy.loginfo("pp_timestamp:%s", timestamp)
            #rospy.loginfo("pp_messages:%s", messages)
            if len(content) != 0:
                _process_packet(content, timestamp, messages)  # call recursively
    if data[0] == 47:  # if packet is a message ("/" = ASCII 47)
        #rospy.loginfo("message type")
        message = _process_message(data)
        #rospy.loginfo("processed_message:%s", message)
        if timestamp != -1:
            message[0] = timestamp
...
def _process_message(data):
    ...
    for type_tag in type_tags:
        #rospy.loginfo("%d_type_tag:%s",i,type_tag)
        #i = i+1
        ...
        if type_tag == ",":  # first character of type tag string
            ...
            rospy.loginfo("Argument type not supported:%s",type_tag)
            break
    return message
