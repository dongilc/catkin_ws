...
public:
			...
			int* dir_vector, double (*inertial_vector)[10], double* friction_vector, double* motor_kt)
	{
		...
		// Publisher
		ctm_pub = nh_.advertise<amg_p1::CTM_Message>("ctm/msgs", 10);
	}
	...
	void InitCTM();
	void RunCTM(double *rps_vesc, double *rad_vesc, double *vel_now, double *target_vel, double *curr_out, double *kp_ctm, double *kd_ctm);
