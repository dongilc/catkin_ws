...
//void CTM::RunCTM(double *erpm, double *vel_now, double *target_vel, double *curr_out, double *kp_ctm, double *kd_ctm)
void CTM::RunCTM(double *rps_vesc, double *rad_vesc, double *vel_now, double *target_vel, double *curr_out, double *kp_ctm, double *kd_ctm)
{
	bool print_ros_info = false; //true; //false;
	...
	for(int i=0; i<NO_OF_WHEEL; i++) {
		...
		rps[i] = Dir[i]*rps_vesc[i];
		rad[i] = mat_Integral(rps[i], rad[i]);	// Calculate position(rad)

		if(print_ros_info)
			ROS_INFO("%d - rotational vel:%.3f(%.3f), rotational pos:%.3f(%.3f)", i, rps[i], rps_vesc[i], rad[i], rad_vesc[i]);
	...
	vel_now[0] = vel[0] = ROW/2.*(rps[0] + rps[1]);
	vel_now[1] = vel[1] = ROW/2./(DCW_X+DCW_Y)*(-rps[0] + rps[1]);
	...
	if(print_ros_info)	ROS_INFO("target vel. - for:%.3f (m/s), rot:%.3f (dps)", target_vel[0], target_vel[1]*RAD2DEG);
	if(print_ros_info)	ROS_INFO("target pos. - for:%.3f (m), rot:%.3f (deg)", target_pos[0], target_pos[1]*RAD2DEG);
	...
	ctm_msg.vel_target.clear();
	ctm_msg.vel_target.push_back(target_vel[0]);
	ctm_msg.vel_target.push_back(target_vel[1]);
	ctm_msg.vel_now.clear();
	ctm_msg.vel_now.push_back(vel[0]);
	ctm_msg.vel_now.push_back(vel[1]);
	ctm_msg.pos_target.clear();
	ctm_msg.pos_target.push_back(target_pos[0]);
	ctm_msg.pos_target.push_back(target_pos[1]);
	ctm_msg.pos_now.clear();
	ctm_msg.pos_now.push_back(pos[0]);
	ctm_msg.pos_now.push_back(pos[1]);
	ctm_msg.current.clear();
	ctm_msg.current.push_back(curr_out[0]);
	ctm_msg.current.push_back(curr_out[1]);
