  ...
  driver_mode_(MODE_INITIALIZING), fw_version_major_(-1), fw_version_minor_(-1)
{
  ...
  state_pub_ = nh.advertise<mjvesc_msgs::MJVescStateStamped>(topic_name.c_str(), 10);
  topic_name = port + "/sensors/customs";
  customs_pub_ = nh.advertise<mjvesc_msgs::MJVescGetCustomApp>(topic_name.c_str(), 10);
...
void VescDriver::vescPacketCallback(const boost::shared_ptr<VescPacket const>& packet)
{
  if (packet->name() == "Values") {
    ...
    mjvesc_msgs::MJVescStateStamped::Ptr state_msg(new mjvesc_msgs::MJVescStateStamped);
    ...
    // todo: publish here
    mjvesc_msgs::MJVescGetCustomApp::Ptr custom_msg(new mjvesc_msgs::MJVescGetCustomApp);
...
//can_id
void VescDriver::setCustomsCallback(const mjvesc_msgs::MJVescSetCustomApp::ConstPtr& custom_set_msg)
{
...
//  }
//}
void VescDriver::dutyCycleCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& duty_cycle)
{
  if (driver_mode_ = MODE_ENABLE) {
...
void VescDriver::currentCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& current)
{
  if (driver_mode_ = MODE_ENABLE) {
...
//}
//cdi
void VescDriver::brakeCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& brake)
{
  if (driver_mode_ = MODE_ENABLE) {
...
//cdi
void VescDriver::speedCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& speed)
{
  if (driver_mode_ = MODE_ENABLE) {
...
void VescDriver::positionCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& position)
{
  if (driver_mode_ = MODE_ENABLE) {
    // ROS uses radians but VESC seems to use degrees. Convert to degrees.
    double position_deg = position_limit_.clip(position->data) * 180.0 / M_PI;
...
void VescDriver::servoCallback(const mjvesc_msgs::MJVescSetCommand::ConstPtr& servo)
{
  if (driver_mode_ = MODE_ENABLE) {
    double servo_clipped(servo_limit_.clip(servo->data));
